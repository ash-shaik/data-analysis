...
/**
  Taking it one step further to do a percentage of overall analysis
 */
...
SELECT customer_id
     ...
     , first_2week_sales
     , ltv
     , CONCAT(ROUND((first_weeks_sales / ltv) * 100), '%') AS first7PctOverall
FROM (
         SELECT customer_tx.*
              , (SELECT SUM(p2.amount)
                 FROM vtrbusic.payment p2
                 WHERE p2.customer_id = customer_tx.customer_id
                   AND p2.payment_date BETWEEN
                     customer_tx.first_payment_date AND customer_tx.first_payment_date + INTERVAL '7 days')  AS first_weeks_sales

              , (SELECT SUM(p2.amount)
                 FROM vtrbusic.payment p2
                 WHERE p2.customer_id = customer_tx.customer_id
                   AND p2.payment_date BETWEEN
                     customer_tx.first_payment_date AND customer_tx.first_payment_date + INTERVAL '14 days') AS first_2week_sales
